...
import org.processmining.plugins.predictive_monitor.bpmn.Evaluator_predictions;
import org.processmining.plugins.predictive_monitor.bpmn.Pair;



public class NotRespondedExistenceAnalyzer implements  DeclareReplayer {
	...
	public boolean checkDataConditions(boolean eventTypes, DataSnapshotListener listener, String activation, String targetName, XTrace trace, String condition){
		...
		for (XEvent event : trace) {
			...
			if(fulfillments.get(traceId).get(activation).get(targetName).size()>0){
				violated = true;
				break;
			}
			index++;
