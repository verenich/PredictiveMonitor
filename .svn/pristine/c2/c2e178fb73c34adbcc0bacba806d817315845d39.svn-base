...
import org.processmining.plugins.predictive_monitor.bpmn.Formula;
import org.processmining.plugins.predictive_monitor.bpmn.FormulaVerificator;
import org.processmining.plugins.predictive_monitor.bpmn.LogReaderAndReplayer;
import org.processmining.plugins.predictive_monitor.bpmn.Pattern;
import org.processmining.plugins.predictive_monitor.bpmn.PatternManager;
import org.processmining.plugins.predictive_monitor.bpmn.SimpleFormula;
import org.processmining.plugins.predictive_monitor.bpmn.XLogReader;
...
public class DiscriminativeSequenceMiner {
	...
	public static ArrayList <Pattern> mineDiscriminativePatterns(XLog log, List<Pattern> frequentPatterns, HashMap<String, Boolean> histTraceFormulaSatisfaction, double minPositiveSupp, double minNegativeSupp){
		...
		try {
			...
			for (String traceId : histTraceFormulaSatisfaction.keySet()) {
				if (histTraceFormulaSatisfaction.get(traceId))
	...
	public static ArrayList<Integer> getAbsoluteSupportValues(XLog log, DataSnapshotListener listener, Set<Integer> traceIndexes, Vector<Formula> formulas){
		...
		for (Integer traceIndex : traceIndexes) {
			if (!FormulaVerificator.isFormulaVerified(log, listener, log.get(traceIndex), formulas))
	...
	public static ArrayList<Integer> getAbsoluteSupportValues(XLog log, HashMap<String, Boolean> histTraceFormulaSatisfaction, ArrayList<XTrace> traces){
		ArrayList<Integer> absoluteSupportValues = new ArrayList<Integer>();
		absoluteSupportValues.add(0);
		absoluteSupportValues.add(0);
		for (XTrace trace : traces) {
			if (histTraceFormulaSatisfaction.get(XConceptExtension.instance().extractName(trace)))
				absoluteSupportValues.set(0, absoluteSupportValues.get(0)+1);
			else 
